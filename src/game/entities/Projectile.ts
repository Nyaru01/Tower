...
export class Projectile {
  ...
  update(dt: number, state: any, audio?: { bass: number }) {
    ...
    if (dist < this.speed * dt) {
      // Audio Bass Shield: Enemies resist damage during bass peaks
      const bassShield = (audio && audio.bass > 0.6) ? 0.6 : 1.0;
      let finalDamage = this.damage * bassShield;
      const isResisted = bassShield < 1.0;
      ...
      if (this.special === 'aoe') {
        ...
        for (const e of state.enemies) {
          if (Math.hypot(e.x - this.target.x, e.y - this.target.y) <= r) {
            if (e.shield > 0) {
              ...
              e.hp -= finalDamage; e.hitFlash = 0.07;
              const now = Date.now();
              state.floatingTexts.push(new FloatingText(e.x, e.y - e.radius - 12, Math.ceil(finalDamage), this.cl));
              if (isResisted && now - (e.lastResistTime || 0) > 500) {
                state.floatingTexts.push(new FloatingText(e.x, e.y - e.radius - 24, "RESIST", 0, '#ff3d5a'));
        ...
        if (this.target.shield > 0) {
          ...
          this.target.hp -= finalDamage; this.target.hitFlash = 0.07;
          if (this.special === 'slow') this.target.slowTimer = 2.8;
          const now = Date.now();
          state.floatingTexts.push(new FloatingText(this.target.x, this.target.y - this.target.radius - 12, Math.ceil(finalDamage), this.cl));
          if (isResisted && now - (this.target.lastResistTime || 0) > 500) {
            state.floatingTexts.push(new FloatingText(this.target.x, this.target.y - this.target.radius - 24, "RESIST", 0, '#ff3d5a'));
            this.target.lastResistTime = now;
          }
